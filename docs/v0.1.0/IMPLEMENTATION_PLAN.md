...
- 接口：
  - **控制面**：管理 VM/镜像/Service（HTTP，localhost）。
...
SDK 提供获取该路径的 API（或控制面提供 `GET /v1/system/paths`）。
...
- 管理镜像与 Service 的“控制面”API（转发到 Guest）。
- 作为 ASP（数据面）网关：
...
- `instance_id` 来源：集成方在 app bundle 资源中提供一个配置文件（例如 `NousAgentRunnerConfig.json`），Runner 启动时读取。
- 若缺失：默认 `instance_id = "default"`（仅用于开发；不建议生产使用）。
...
3. 使用 token 调用控制面创建 service，再用 ASP 走数据面（v1 暂定 WebSocket）。
...
- 控制面 `POST /v1/services` / `DELETE /v1/services` 可用。
...
- `NousAgentRunnerKit`：启动/发现 runner、封装 control/data plane。
...
原则：**不把 Lima 的 CLI/配置暴露给集成方**；集成方只面向 `nous-agent-runnerd` 的 Control Plane / ASP。

落地方式：

1. `nous-agent-runnerd` 打包 `limactl`（放在 App bundle 内），运行时设置私有 `LIMA_HOME`：  
   `~/Library/Application Support/NousAgentRunner/<instance_id>/lima/`（确保不同 App/不同 worktree 不冲突）。
...
4. 对外 Control Plane / ASP 始终由 `nous-agent-runnerd` 提供；避免把运行时行为绑死在 `limactl shell` / `lima nerdctl` 这类“命令封装”上（调试可用，但不作为产品依赖）。
...
本仓库包含 `references/goose`（Block 开源的本地 Agent，Rust 实现）。对 Nous Agent Runner 的直接复用价值有限（Goose 是“Agent 产品”，而我们是“Agent 运行时/隔离层”），但有两块非常值得参考：
...
- 协议服务端：`references/goose/crates/goose-acp/src/server.rs`
- 简易客户端（很适合拿来理解协议行为）：`references/goose/test_acp_client.py`
...
- SSE 流式（reply）：`references/goose/crates/goose-server/src/routes/reply.rs`
- OpenAPI 生成入口：`references/goose/crates/goose-server/src/openapi/`、`references/goose/crates/goose-server/src/bin/generate_schema.rs`
