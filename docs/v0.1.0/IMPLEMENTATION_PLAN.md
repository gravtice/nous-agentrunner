# Agent Runner v1（MVP）设计与实现计划

## 0. 文档目的

本文档定义 Agent Runner v1（MVP）的目标、边界、架构、接口协议与实现分解，作为后续实现与验收的唯一参考。
...
- v1 采用 **Lima 子进程 + gRPC port forwarder（底层 vsock）** 连接 Guest 内 `guest-runnerd`；不再依赖 `ssh.config` + SSH port-forward。
...
- `instance_id` 来源优先级（零参数）：
  1) 若存在：集成方在 app bundle 资源中提供 `AgentRunnerConfig.json`，Runner 启动时读取。
...
- `AGENT_RUNNER_PORT`：对外监听端口（未配置则首次运行自动分配并写入 `.env.local`）。
- `AGENT_RUNNER_REGISTRY_BASE`：官方 registry base（固定编译默认值，可通过文件覆盖）。
- `AGENT_RUNNER_VM_MEMORY_MB`：VM 内存（默认例如 4096）。
- `AGENT_RUNNER_VM_CPU_CORES`：VM CPU（默认例如 4）。
...
```json
{"ref":"docker.io/gravtice/agent-runner-claude-agent-service:0.2.4"}
```

约束：

- `ref` 会先做 canonicalize（例如 `gravtice/...` 视为 `docker.io/gravtice/...`），再校验其必须以 `AGENT_RUNNER_REGISTRY_BASE` 为前缀，否则拒绝（`REGISTRY_NOT_ALLOWED`）。
...
{
  "type": "claude",
  "image_ref": "docker.io/gravtice/agent-runner-claude-agent-service:0.2.4",
...
- **会话语义简单**：一个 `service_id` 固定一个 `session_id`（断线重连复用；用于 Claude Agent SDK 的 session 续聊与 ASMP resume）。
...
- 一个 `service_id` 固定一个会话（`session_id`）；WS 连接是传输通道，可断线重连续聊。
- 同一 `service_id` 同时只允许一条 WS 连接（避免多客户端共享同一 Agent session 造成踩踏）。
...
- `source.type="path"` 时必须在 Share 白名单内，否则返回 `PATH_NOT_ALLOWED`（该条 `input` 失败，连接保持可用）。
- `source.type="bytes"` 时解码后大小必须 <= `MAX_INLINE_BYTES=8MB`，否则返回 `INLINE_BYTES_TOO_LARGE`（该条 `input` 失败，连接保持可用）。
...
- Host → Guest：通过 Lima hostagent 的 **gRPC port forwarder（VZ 下 guestagent 连接走 vsock）** 将 Guest `127.0.0.1:<AGENT_RUNNER_GUEST_RUNNERD_PORT>` 映射到 Host `127.0.0.1:<AGENT_RUNNER_GUEST_FORWARD_PORT>`。
...
- Host 进程（agent-runnerd）可信，负责鉴权与路径校验。
...
- `agent-runnerd`（Host daemon）
- VM 资源（kernel/initrd/disk 模板/guest runner 等）
-（可选）`AgentRunnerKit`（Swift SDK）

App 启动后由 SDK/集成代码负责：

1. 启动 `agent-runnerd`（子进程或 launchd，v1 建议子进程方式）。
...
- 定义目录结构、构建系统、基础日志。
- `agent-runnerd` 能启动并提供 `GET /v1/system/status`。

### M1：共享 VM（子进程集成 Lima）

- 在 `agent-runnerd` 内以子进程方式调用 `limactl` 管理 VM 生命周期（start/stop/status）。
- 生成并落盘 lima YAML（`vmType: vz`、`mountType: virtiofs`、`mounts` 由 Share 白名单生成且同路径挂载）。
- VM 健康检查（确保 `guest-runnerd` 已就绪）。

### M2：Guest 容器管理与最小 Service

- 通过 Lima `provision` 在 Guest 内安装/启动 `guest-runnerd`（systemd service）。
- `guest-runnerd` 集成 `containerd`，可启动一个最小“echo service”（用于端到端冒烟测试）。
...
- `AgentRunnerKit`：启动/发现 runner、封装 ASMP/ASP。
...
原则：**不把 Lima 的 CLI/配置暴露给集成方**；集成方只面向 `agent-runnerd` 的 ASMP / ASP。
...
3. 在 Guest 内安装/启动 `guest-runnerd`（systemd），并作为 Host ↔ Guest 的稳定控制入口（通过 Lima `portForwards` 访问；由 Lima provision/cloud-init 安装与启动）。
4. 对外 ASMP / ASP 始终由 `agent-runnerd` 提供；避免把运行时行为绑死在 `limactl shell` / `lima nerdctl` 这类“命令封装”上（调试可用，但不作为产品依赖）。
...
### 17.2 Agent Runner（Host/Guest daemon）
...
本仓库不再 vendoring Goose（Block 开源的本地 Agent，Rust 实现；参考：https://github.com/block/goose ）。对 Agent Runner 的直接复用价值有限（Goose 是“Agent 产品”，而我们是“Agent 运行时/隔离层”），但有两块非常值得参考：
