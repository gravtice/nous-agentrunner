# OpenClaw 集成 Nous Agent Runner（方案记录 / Codex）

> 结论：可行，且建议 **通过 OpenClaw 插件（plugin）实现**，避免改 OpenClaw 核心。
...
本方案目标是：让 OpenClaw 在不侵入其核心 Agent Runner（Pi/CLI）实现的前提下，**把 Nous Agent Runner 当作“外部隔离子代理”**进行调用，从而获得：

- 更强的隔离（VM + 容器）
- 更明确的文件访问边界（Share 白名单 + rw_mounts）
- 可把重任务/高风险工具执行放到隔离环境

## 1. OpenClaw 现状观察（可用扩展点）

OpenClaw 已有成熟的插件系统，可用于“新增工具/命令/服务”：

- 插件 API 类型定义：`temp/openclaw/src/plugins/types.ts`
  - `registerTool(...)`：注册 Agent tool
  - `registerCommand(...)`：注册 `/command`（绕过 LLM，优先于内置命令/Agent）
  - `registerService(...)`：注册后台服务（可选）
- 插件 tool 的 allowlist/optional gating：`temp/openclaw/src/plugins/tools.ts`
  - `optional: true` 的 tool 默认不暴露给模型，必须被 allowlist 明确放行
  - 这点非常适合作为“高风险/高权限/外部执行”能力的默认防线
- 插件命令注册与保留命令拦截：`temp/openclaw/src/plugins/commands.ts`
- 现成“插件注册工具”的最小范例：`temp/openclaw/extensions/llm-task/index.ts`

结论：**把 Nous Agent Runner 适配成一个 OpenClaw plugin**，并以“可选 tool + 可选命令”的方式暴露，是最小改动、最符合 OpenClaw 现有治理（allowlist / policy）的集成路径。

## 2. Nous Agent Runner 能提供什么（对 OpenClaw 的增益）
...
### 3.1 做成一个 OpenClaw 插件：`nous-agent-runner`

插件提供两种入口（都可做成 optional + allowlist）：

1) Tool：`nous.run`（给 OpenClaw 主 Agent 调用）
...
1. 发现/拉起 Runner：用 `nous-agent-runner-sdk` 的 `NousAgentRunnerDaemon.ensureRunning()`（优先），或 `NousAgentRunnerRuntime.discover()`
2. 创建临时 service：`client.createClaudeService(...)`
3. 连接 ASP WS：`client.openChatWebSocket(serviceId)`，发送 `input`，流式收 `response.delta`/`response.final`，等待 `done`
4. 清理：`client.deleteService(serviceId)`
...
### 4.2 Tool 暴露策略（依赖 OpenClaw 现成治理）

- 将 `nous.run` 注册为 `optional: true`，默认不提供给模型
- 只有当用户在 OpenClaw config 的 tools allowlist 中显式加入该 tool，才允许模型调用
...
- 插件日志中做敏感信息脱敏
- 优先支持从 OpenClaw 的 auth profiles/配置中读取（若后续要做深度集成）

## 5. 建议的对外能力形态（让 OpenClaw“变强”但不乱）
...
- 当主 OpenClaw agent 的工具策略严格时，把“危险动作”委派给 `nous.run`，并要求其以 `plan` 输出 + 由主 agent 二次确认

## 6. 实施步骤（最小可交付）

### Stage A：PoC（1~2 天量级）

- 新建 OpenClaw 插件目录（或 workspace plugin），包含：
  - `openclaw.plugin.json`（config schema）
  - `index.ts` 注册 tool + `/nous` 命令
- 引入 TS SDK（替代自研 ASMP/ASP 客户端）：
  - 依赖：`npm install nous-agent-runner-sdk`
  - 运行时发现/拉起：`NousAgentRunnerDaemon.ensureRunning()`
  - ASMP 调用：`NousAgentRunnerClient`（create/list/delete service 等）
  - ASP WS：`openChatWebSocket()`（SDK 内部直接依赖 `ws`，需要 Node/Electron main process）
- 输出格式：
...
- 错误码分级与重试策略（ASMP/ASP）
- 单元测试（mock ASMP/ASP server；或对 SDK 做最小 stub，再重点测试插件编排逻辑）

## 7. 未决问题（需要你拍板）

1) OpenClaw 与 Runner 的部署关系：
   - 只支持 macOS Apple Silicon（Runner v1 平台限制），还是要做“插件可选启用 + 其它平台降级”为纯本地 OpenClaw？
2) service 类型：
