...
- `skills`：可选。若为空/缺省：安装所有发现到的 skills；否则只安装列表内匹配的 skill（按目录名匹配，大小写不敏感）。
...
### 2.3 Source 格式（兼容 vercel-labs/skills 的常用输入）
...
### 2.4 Skills 发现规则（兼容 vercel-labs/skills 的 discoverSkills）
