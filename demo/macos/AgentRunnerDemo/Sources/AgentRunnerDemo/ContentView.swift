...
struct ContentView: View {
    ...
    @AppStorage("agent_runner.demo.service_env") private var serviceEnvText = ""
    @AppStorage("agent_runner.demo.max_thinking_tokens") private var maxThinkingTokensText = "8000"
    @AppStorage("agent_runner.demo.idle_timeout_seconds") private var idleTimeoutSecondsText = "0"
    ...
    @State private var daemon: AgentRunnerDaemon?
    ...
    private func parseJSONObjectText(_ text: String) throws -> [String: Any] {
        guard let data = text.data(using: .utf8) else {
            throw AgentRunnerError.invalidConfig("invalid utf-8")
        }
        let obj = try JSONSerialization.jsonObject(with: data)
        guard let dict = obj as? [String: Any] else {
            throw AgentRunnerError.invalidConfig("json must be an object")
        }
        return dict
    }
    ...
    private func client() throws -> AgentRunnerClient {
        let runnerContext = try AgentRunnerContext.discover()
        return AgentRunnerClient(context: runnerContext)
    }
        ...
        do {
            ...
            if !maxThinkingValue.isEmpty {
                guard let n = Int(maxThinkingValue), n > 0 else {
                    throw AgentRunnerError.invalidConfig("max_thinking_tokens must be a positive integer")
                }
    ...
    private func allocateWorkDir(client: AgentRunnerClient) async throws -> String {
        let paths = try await client.getSystemPaths()
        guard let tmpDir = paths["default_temp_dir"] as? String, !tmpDir.isEmpty else {
            throw AgentRunnerError.invalidConfig("missing default_temp_dir")
        }
    ...
    private func prepareSelectedWorkDir(url: URL) throws -> String {
        ...
        guard FileManager.default.fileExists(atPath: url.path, isDirectory: &isDir), isDir.boolValue else {
            throw AgentRunnerError.invalidConfig("selected work dir is not a directory: \(url.path)")
        }
        return url.path
    }
    ...
    private func sendAskAnswer(askID: String, answers: [String: String]) {
        ...
        do {
            ...
            guard let json = String(data: data, encoding: .utf8) else {
                throw AgentRunnerError.io("failed to encode json")
            }
    ...
    private func parseEnvText(_ text: String) throws -> [String: String] {
        ...
        for (idx, rawLine) in lines.enumerated() {
            ...
            guard let eq = line.firstIndex(of: "=") else {
                throw AgentRunnerError.invalidConfig("env line \(idx + 1): missing '='")
            }
            ...
            if key.isEmpty {
                throw AgentRunnerError.invalidConfig("env line \(idx + 1): empty key")
            }
            out[key] = value
        }
        return out
