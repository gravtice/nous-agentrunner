...
import { isSafeSkillDirName, AgentRunnerContext } from "./context";
...
export class AgentRunnerClient {
  private readonly runnerContext: AgentRunnerContext;
  ...
  constructor(
    ...
    if (!f) {
      throw new AgentRunnerError("invalidConfig", "fetch is not available");
    }
    this.fetchFn = f;
  }
  ...
  discoverSkills(source: string, opts: { ref?: string; subpath?: string } = {}) {
    ...
    if (trimmed.length === 0) {
      throw new AgentRunnerError("invalidConfig", "source is required");
    }
  ...
  deleteSkill(name: string) {
    ...
    if (trimmed.length === 0 || !isSafeSkillDirName(trimmed)) {
      throw new AgentRunnerError("invalidConfig", "invalid skill name");
    }
  ...
  private async requestJSON(
    ...
    try {
      ...
      if (resp.status !== 200) {
        throw new AgentRunnerError("http", `http ${resp.status}`, {
          status: resp.status,
          body: text,
        });
      }
      ...
      if (err && typeof err === "object" && (err as { name?: unknown }).name === "AbortError") {
        throw new AgentRunnerError("timeout", "request timeout");
      }
      throw new AgentRunnerError(
        "io",
