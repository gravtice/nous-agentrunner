...
export class NousAgentRunnerRuntime {
  ...
  static async discover(opts: { instanceId?: string } = {}) {
    ...
    if (!isSafeInstanceId(instanceId)) {
      throw new NousAgentRunnerError("invalidConfig", "invalid instance id");
    }
...
export function deriveInstanceIdFromBundleId(bundleId: string): string {
  ...
  if (normalized.length === 0) {
    throw new NousAgentRunnerError("invalidConfig", "bundle id is empty");
  }
...
export async function discoverInstanceId(): Promise<string> {
  ...
  const configCandidates = [
    path.join(path.dirname(exe), "NousAgentRunnerConfig.json"),
    path.resolve(path.dirname(exe), "..", "Resources", "NousAgentRunnerConfig.json"),
  ];
...
function resolveHomeDir(): string {
  ...
  if (!home) {
    throw new NousAgentRunnerError("io", "missing home directory");
  }
  return home;
}
...
async function loadPort(appSupportDir: string): Promise<number> {
  ...
  for (const name of candidates) {
    ...
    const portStr = parseEnv(contents)["NOUS_AGENT_RUNNER_PORT"];
    if (!portStr) continue;
  ...
  throw new NousAgentRunnerError(
    "missingConfig",
    "NOUS_AGENT_RUNNER_PORT not found",
  );
}
...
async function loadToken(appSupportDir: string): Promise<string> {
  ...
  if (token.length === 0) {
    throw new NousAgentRunnerError("missingConfig", "token file not found");
  }
  return token;
}
