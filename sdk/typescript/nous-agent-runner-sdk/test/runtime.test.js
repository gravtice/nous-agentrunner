...
test("parseEnv: strips quotes, ignores comments", () => {
  const env = parseEnv(`
# comment
NOUS_AGENT_RUNNER_PORT=1234
FOO="bar"
BAR='baz'
`);
  assert.equal(env.NOUS_AGENT_RUNNER_PORT, "1234");
...
test("resolveAppSupportDir uses HOME", () => {
  ...
  try {
    assert.equal(
      ...
      "/tmp/home/Library/Application Support/NousAgentRunner/inst",
    );
  } finally {
...
test("NousAgentRunnerRuntime.discover uses .env.local over .env.production", async () => {
  ...
  try {
    ...
    await fs.writeFile(
      ...
      "NOUS_AGENT_RUNNER_PORT=1111\n",
      "utf8",
    );
    await fs.writeFile(
      path.join(appSupportDir, ".env.local"),
      "NOUS_AGENT_RUNNER_PORT=2222\n",
      "utf8",
    );
    await fs.writeFile(path.join(appSupportDir, "token"), "tok\n", "utf8");
